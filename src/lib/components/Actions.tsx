...
import { PageBuilderWidget } from "..";

export const RpbActions: React.FC<{
  canMove?: boolean;
  canDelete?: boolean;
  canExpand?: boolean;
  hasSettings?: boolean;
  ...
  <span className="rpb-actions" onClick={e => e.stopPropagation()}>
    {actionRenderer && actionRenderer()}
    {canMove && (
      <a
        className="move"
        onMouseEnter={() => enableDrag!(true)}
    ...
    {hasSettings && (
      <a className="settings">
        ...
        <div className="rpb-actions__settings">{children}</div>
      </a>
    )}
    {canExpand && (
      <a onClick={onToggleExpand}>
        <svg width="24" height="24" viewBox="0 0 24 24">
          <path
            d={
              isExpanded
